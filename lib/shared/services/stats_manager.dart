...
class StatsManager {
  ...
  final ValueNotifier<UserStats> statsNotifier = ValueNotifier(
    UserStats(streak: 0, solved: 0, merit: 0),
  );
  ...
  Future<void> init() async {
    ...
    final Map<String, int> modeFrequency = decodedModeFreq.map(
      (k, v) => MapEntry(k, v as int),
    );

    final historyRaw = _prefs.getString('stat_game_history') ?? '[]';
    final List<dynamic> decodedHistory = jsonDecode(historyRaw);
    final List<Map<String, dynamic>> gameHistory = decodedHistory
  ...
  Future<int> recordWin({
    ...
    await _prefs.setString(
      'stat_dist',
      jsonEncode(newDist.map((k, v) => MapEntry(k.toString(), v))),
    );
  ...
  Future<void> applyMajorBonusBoost() async {
    ...
    final bool allMajorsUnlocked =
        current.unlockedIds.length >= MajorsData.all.length;
    ...
    statsNotifier.value = current.copyWith(extraBoosts: newBoostCount);
  }
  ...
  Future<Map<String, int>> _incrementModeFrequency(
    int length,
    int maxAttempts,
  ) async {
    final current = statsNotifier.value;
